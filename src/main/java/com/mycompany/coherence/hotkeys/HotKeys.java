...
public class HotKeys {
    ...
    private Map<Member, SortedCollectionWithCapacity<HotKeyData<Integer>>> invocationResults = new HashMap<>();
	
    static {
        cliOptions = new Options();
        cliOptions.addOption(new Option("help", "print command line usage"));
        cliOptions.addOption("n", true, String.format("optional number of maximum hot keys to fetch; defaults to %d", DEFAULT_TOP_N));
        cliOptions.addOption("c", true, String.format("optional name of cache; defaults to %s", DEFAULT_CACHE_NAME));
        cliOptions.addOption("d", true, String.format("optional name of DistributedCache service; defaults to %s", DEFAULT_CACHE_SERVICE_NAME));
        cliOptions.addOption("i", true, String.format("optional name of InvocationService; defaults to %s", DEFAULT_INVOCATION_SERVICE_NAME));
	...
	public HotKeys(String cacheName, String distributedCacheServicename, String invocationServiceName, int topN, boolean verbose) {
		super();
	...
	/**
	 * Orchestrate collection of top n keys of partitioned cache contents as configured by constructor parameters.
	 */
	private void fetch() {
		
        if (getStorageMembers().size() == 0) {
            throw new IllegalStateException("There must be storage members in the cluster for fetch top n keys to run.");
        }
        
        if(verbose) showStorageMembers(getStorageMembers());
    ...
    protected Map<Member, Invocable> createTasks()
    {
        ...
        for (Member member : storageMembers)
        {
        	HotKeyInvocable<Integer> task = new HotKeyInvocable<Integer>(getCacheName(), getCacheServiceName(), getTopN());
    ...
    private void reportResults(long executionTime) {
    	...
    	SortedCollectionWithCapacity<HotKeyData<Integer>> mergedResult = new SortedCollectionWithCapacity<HotKeyData<Integer>>(getTopN());
    	...
    	for (Map.Entry<Member, SortedCollectionWithCapacity<HotKeyData<Integer>>> entry : invocationResults.entrySet())
        {
            ...
            if (entry.getValue() != null) {
            	...
            	System.out.println("Execution on member "+getShortMemberInfo(member)+" took "+ result.getElapsed() + " ms");
    ...
    private String getShortMemberInfo(Member member) {
		// TODO Auto-generated method stub
		return "Member(Id"+member.getId()+", Address="+member.getAddress().getHostAddress()+":"+member.getPort()+", Role="+member.getRoleName()+")";
	}

	/**
     * Create and return an InvocationObserver for observing the progress of asynchronous Invocable execution.
     */
    private InvocationObserver newInvocationObserver(final CountDownLatch countDownLatch, final long startTime, final boolean isVerbose)
    {
        return new InvocationObserver()
        {
            ...
            {
            	if(isVerbose) System.out.println("invocation completed");
            }
        };
    }
