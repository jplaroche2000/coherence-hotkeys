...
public class HotKeyInvocable<K extends Comparable<K>> extends AbstractInvocable
	...
	public void run() {
		...
		SortedCollectionWithCapacity<HotKeyData<K>> results = new SortedCollectionWithCapacity<HotKeyData<K>>(this.topN);
		...
		BackingMapContext backingMapContext = cacheService.getBackingMapManager().getContext().getBackingMapContext(cacheName);
		Converter converter = backingMapContext.getManagerContext().getKeyFromInternalConverter();

		if (logger.isDebugEnabled()) {
			logger.debug("There are " + backingMapEntries.size() + " entries in the " + cacheName + " cache on local member id " + localMemberId);
		}
		...
		for (Object entry : backingMapEntries) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("key=" + converter.convert(localCacheEntry.getKey()) + ", touchCount=" + localCacheEntry.getTouchCount());
			}
		}
